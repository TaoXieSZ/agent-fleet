...
from agent_fleet import cmux_control as cc
from agent_fleet.cmux_control import (
    BOARD_MARKER,
    NATO,
    CmuxClient,
    Session,
...
SURF = {
    "WSA": json.dumps({"surfaces": [
        {"id": "SB", "ref": "surface:20", "index": 0, "type": "terminal",
         "title": "cd '/x/tools' && python3 -m agent_fleet.board --watch",
         "focused": False},
...
def test_build_excludes_board_and_browser():
    ...
    assert BOARD_MARKER == "agent_fleet.board"
...
def test_build_board_rows_from_client():
    from agent_fleet.board import build_board
