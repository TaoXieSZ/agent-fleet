...
if [ -n "$SURF_ID" ]; then
  BOARD_CMD="cd '$TOOLS_DIR' && python3 -m agent_fleet.board --watch --self-surface $SURF_ID"
...
echo "  - board : terminal, $TOOLS_DIR  ->  python -m agent_fleet.board --watch --self-surface $SURF_ID"
